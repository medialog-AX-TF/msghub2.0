...
메시지허브 AI 어시스턴트는 다양한 LLM 모델을 지원하며, .env 환경 설정 파일을 통해 필요에 따라 모델을 선택할 수 있습니다:
...
모델 선택은 .env 환경 설정 파일에서 변경할 수 있으며, 각 모델의 특성에 따라 응답 품질, 속도, 비용이 달라질 수 있습니다. 환경 변수를 통해 모델 전환 및 설정 관리가 가능합니다.
...
6. 런타임 환경 변수 변경 기능 (서버 재시작 없이 모델 전환)
